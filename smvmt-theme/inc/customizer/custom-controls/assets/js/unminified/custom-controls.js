( function( $ ) {
	...
	wp.customize.controlConstructor['smvmt-background'] = wp.customize.Control.extend({
		...
		initAstBgControl: function() {
			...
			var control = this,
				...
				picker  = control.container.find( '.smvmt-color-control' );
			...
			control.container.on( 'click', '.background-image-upload-remove-button', function( e ) {
				...
				control.container.find( '.more-settings' ).find('.message').html( smvmtCustomizerControlBackground.moreSettings );
				control.container.find( '.more-settings' ).find('.icon').html( '↓' );

				if ( preview.length ) {
					preview.removeClass().addClass( 'placeholder' ).html( smvmtCustomizerControlBackground.placeholder );
				}
				if ( removeButton.length ) {
					removeButton.hide();
				}
			...
			control.container.on( 'click', '.more-settings', function( e ) {
				...
				if( 'down' === $(this).attr( 'data-direction' ) )
				{
					...
					$(this).find('.message').html( smvmtCustomizerControlBackground.lessSettings );
					$(this).find('.icon').html( '↑' );
				} else {
					$(this).attr('data-direction', 'down');
					$(this).find('.message').html( smvmtCustomizerControlBackground.moreSettings );
					$(this).find('.icon').html( '↓' );
				}
			});
		},
...
( function( $ ) {
	...
	wp.customize.controlConstructor['smvmt-color'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			this.container.find('.smvmt-color-picker-alpha' ).wpColorPicker({
				/**
...
( function( $ ) {
	...
	wp.customize.controlConstructor['smvmt-link'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			// Save the value.
			this.container.on( 'change keyup', '.smvmt-link-input', function(e) {
				e.preventDefault();
			...
			// Save the value.
			this.container.on( 'change click', '.smvmt-link-open-in-new-tab', function() {
			...
			// Save the value.
			this.container.on( 'change keyup', '.smvmt-link-relationship', function(e) {
				e.preventDefault();
...
wp.customize.controlConstructor['smvmt-customizer-link'] = wp.customize.Control.extend({
...
wp.customize.controlConstructor['smvmt-radio-image'] = wp.customize.Control.extend({
...
( function( $ ) {
	 ...
	 * @package smvmt
	 */
	wp.customize.controlConstructor['smvmt-responsive-color'] = wp.customize.Control.extend({
		...
		initAstBgControl: function() {
			...
			var control = this,
				value   = control.setting._value,
				picker  = control.container.find( '.smvmt-responsive-color' );
			...
			jQuery( '.customize-control-smvmt-responsive-color .customize-control-content .smvmt-responsive-color.desktop' ).parents( '.wp-picker-container' ).addClass( 'active' );
	...
	jQuery('.wp-full-overlay-footer .devices button').on('click', function() {
		...
		jQuery( '.customize-control-smvmt-responsive-color .customize-control-content .wp-picker-container' ).removeClass( 'active' );
		jQuery( '.customize-control-smvmt-responsive-color .customize-control-content .smvmt-responsive-color.' + device ).parents( '.wp-picker-container' ).addClass( 'active' );
	});
})(jQuery);
...
( function( $ ) {
	...
	wp.customize.controlConstructor['smvmt-responsive-slider'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			this.container.on( 'input change', 'input[type=range]', function() {
				var value 		 = jQuery( this ).val(),
					input_number = jQuery( this ).closest( '.input-field-wrapper' ).find( '.smvmt-responsive-range-value-input' );
			...
			this.container.on('click', '.smvmt-responsive-slider-reset', function() {

				var wrapper 		= jQuery( this ).parent().find('.input-field-wrapper.active'),
					input_range   	= wrapper.find( 'input[type=range]' ),
					input_number 	= wrapper.find( '.smvmt-responsive-range-value-input' ),
		...
		updateValue: function() {
			...
			control.responsiveContainer.find( '.smvmt-responsive-range-value-input' ).each( function() {
				var responsive_input = jQuery( this ),
				item = responsive_input.data( 'id' ),
				item_value = responsive_input.val();
		...
		smvmtResponsiveInit : function() {
			...
			this.container.on( 'click', '.smvmt-responsive-slider-btns button', function( event ) {
	...
	jQuery(' .wp-full-overlay-footer .devices button ').on('click', function() {
		...
		jQuery( '.customize-control-smvmt-responsive-slider .input-field-wrapper, .customize-control .smvmt-responsive-slider-btns > li' ).removeClass( 'active' );
		jQuery( '.customize-control-smvmt-responsive-slider .input-field-wrapper.' + device + ', .customize-control .smvmt-responsive-slider-btns > li.' + device ).addClass( 'active' );
	});
...
( function( $ ) {
    ...
    wp.customize.controlConstructor['smvmt-settings-group'] = wp.customize.Control.extend({
        ...
        registerToggleEvents: function() {
            ...
            $( '.wp-full-overlay-sidebar-content, .wp-picker-container' ).click( function( e ) {
                if ( ! $( e.target ).closest( '.smvmt-field-settings-modal' ).length ) {
                    $( '.smvmt-adv-toggle-icon.open' ).trigger( 'click' );
                }
            });

            control.container.on( 'click', '.smvmt-toggle-desc-wrap .smvmt-adv-toggle-icon', function( e ) {
                ...
                var parent_wrap = $this.closest( '.customize-control-smvmt-settings-group' );
                var is_loaded = parent_wrap.find( '.smvmt-field-settings-modal' ).data('loaded');
                var parent_section = parent_wrap.parents('.control-section');

                if( $this.hasClass('open') ) {
                    parent_wrap.find( '.smvmt-field-settings-modal' ).hide();
                } else {
                    /* Close popup when another popup is clicked to open */
                    var get_open_popup = parent_section.find('.smvmt-adv-toggle-icon.open');
                    if( get_open_popup.length > 0 ) {
                        get_open_popup.trigger('click');
                    }
                    if( is_loaded ) {
                        parent_wrap.find( '.smvmt-field-settings-modal' ).show();
                    } else {
                        var fields = control.params.smvmt_fields;

                        var $modal_wrap = $( smvmt.customizer.group_modal_tmpl );

                        parent_wrap.find( '.smvmt-field-settings-wrap' ).append( $modal_wrap );
                        parent_wrap.find( '.smvmt-fields-wrap' ).attr( 'data-control', control.params.name );
                        control.smvmt_render_field( parent_wrap, fields, control );
                        ...
                        if( 'mobile' == device ) {
                            jQuery('.smvmt-responsive-btns .mobile, .smvmt-responsive-slider-btns .mobile').addClass('active');
                            jQuery('.smvmt-responsive-btns .preview-mobile, .smvmt-responsive-slider-btns .preview-mobile').addClass('active');
                        } else if( 'tablet' == device ) {
                            jQuery('.smvmt-responsive-btns .tablet, .smvmt-responsive-slider-btns .tablet').addClass('active');
                            jQuery('.smvmt-responsive-btns .preview-tablet, .smvmt-responsive-slider-btns .preview-tablet').addClass('active');
                        } else {
                            jQuery('.smvmt-responsive-btns .desktop, .smvmt-responsive-slider-btns .desktop').addClass('active');
                            jQuery('.smvmt-responsive-btns .preview-desktop, .smvmt-responsive-slider-btns .preview-desktop').addClass('active');
                        }
                    }
                }
            ...
            control.container.on( "click", ".smvmt-toggle-desc-wrap > .customizer-text", function( e ) {

                e.preventDefault();
                e.stopPropagation();

                jQuery(this).find( '.smvmt-adv-toggle-icon' ).trigger('click');
            });
        },
        ...
        ast_render_field: function( wrap, fields, control_elem ) {
            ...
            if( 'undefined' != typeof fields.tabs ) {
                ...
                fields_html += '<div id="' + clean_param_name + '-tabs" class="smvmt-group-tabs">';
                fields_html += '<ul class="smvmt-group-list">';
                var counter = 0;
                ...
                fields_html += '<div class="smvmt-tab-content" >';
            ...
            _.each( control_types, function( control_type, index ) {
                ...
                switch( control_type.key ) {
                    ...
                    case "smvmt-font":
                        ...
                        var googleFontsString = smvmt.customizer.settings.google_fonts;
                        control.container.find( '.smvmt-font-family' ).html( googleFontsString );

                        control.container.find( '.smvmt-font-family' ).each( function() {
                            ...
                            var fontWeightContainer = jQuery(".smvmt-font-weight[data-connected-control='" + optionName + "']");
                        ...
                        control.container.find( '.smvmt-font-family' ).selectWoo();
                        control.container.find( '.smvmt-font-family' ).on( 'select2:select', function() {
                        ...
                        control.container.find( '.smvmt-font-weight' ).on( 'change', function() {
                    ...
                    case "smvmt-select":
                        ...
                        control.container.on( 'change', '.smvmt-select-input', function() {
                    ...
                    case "smvmt-slider":
                        ...
                        control.container.on('input change', 'input[type=range]', function () {
                            var value = jQuery(this).attr('value'),
                                input_number = jQuery(this).closest('.wrapper').find('.SMVMT_range_value .value');
                        ...
                        // Handle the reset button.
                        control.container.on( 'click', '.smvmt-slider-reset', function () {

                            var wrapper = jQuery(this).closest('.wrapper'),
                                input_range = wrapper.find('input[type=range]'),
                                input_number = wrapper.find('.SMVMT_range_value .value'),
                                default_value = input_range.data('reset_value');
                        ...
                        // Save changes.
                        control.container.find( '.customize-control-smvmt-slider' ).on('input change', 'input[type=number]', function () {
            ...
            wrap.find( '.smvmt-field-settings-modal' ).data( 'loaded', true );
        ...
        generateFieldHtml: function ( fields_data, field_values ) {
            ...
            _.each(fields_data, function (attr, index) {
                ...
                new_value = ( wp.customize.control( 'smvmt-settings['+attr.name+']' ) ? wp.customize.control( 'smvmt-settings['+attr.name+']' ).params.value : '' );
        ...
        generateDropdownHtml: function( weightObject, element ) {
            ...
            smvmtTypo[ 'inherit' ] = currentWeightTitle;
            ...
            for ( ; counter < weightObject.length; counter++ ) {
                ...
                if( ! weightObject[ counter ].includes( "italic" ) ){
                    weightOptions += '<option value="' + weightObject[ counter ] + '"' + selected + '>' + smvmtTypo[ weightObject[ counter ] ] + '</option>';
                }
            }
        ...
        initResponsiveTrigger: function( wrap, control_elem ) {
            ...
            wrap.find('.smvmt-responsive-btns button').on('click', function (event) {
        ...
        initColor: function ( wrap, control_elem, name ) {
            ...
            var control = this;
            var picker = wrap.find('.customize-control-smvmt-color .smvmt-color-picker-alpha');
        ...
        initResponsiveColor: function( wrap, control_elem, name ) {
            ...
            var control = this;
            var picker = wrap.find( '.smvmt-responsive-color' );
                ...
                change: function(event, ui) {
                    ...
                    if ('undefined' != typeof event.originalEvent || 'undefined' != typeof ui.color._alpha) {
                        if ( jQuery('html').hasClass('responsive-background-color-ready') ) {
                            ...
                            var stored = {
                                'desktop' : jQuery( ".desktop.smvmt-responsive-color[data-name='"+ option_name +"']" ).val(),
                                'tablet'  : jQuery( ".tablet.smvmt-responsive-color[data-name='"+ option_name +"']" ).val(),
                                'mobile'  : jQuery( ".mobile.smvmt-responsive-color[data-name='"+ option_name +"']" ).val()
                            };
            ...
            wrap.find( '.smvmt-responsive-color.desktop' ).parents( '.wp-picker-container' ).addClass( 'active' );
        },

        onOptionChange:function ( e, control, element, value, name ) {

            var control_id  = $( '.hidden-field-smvmt-settings-' + name );
            control_id.val( value );
            sub_control = wp.customize.control( "smvmt-settings[" + name + "]" );
        ...
        updateResonsiveValue: function( element, name ) {
            ...
            // Set the spacing container.
            control.responsiveContainer = element.closest( '.smvmt-responsive-wrapper' );
        ...
        initAstResonsiveBgControl: function( control, control_atts, name ) {
            ...
            var picker           = control.container.find('.smvmt-responsive-bg-color-control');
            ...
            controlContainer.on('click', '.background-image-upload-remove-button', function (e) {
                ...
                controlContainer.find('.background-container.' + screen + ' .more-settings').find('.message').html(smvmtCustomizerControlBackground.moreSettings);
            ...
            controlContainer.find('.smvmt-responsive-btns button').on('click', function (event) {
            ...
            jQuery(' .wp-full-overlay-footer .devices button ').on('click', function () {
                ...
                jQuery('.customize-control-smvmt-responsive-background .background-container, .customize-control .smvmt-responsive-btns > li').removeClass('active');
                jQuery('.customize-control-smvmt-responsive-background .background-container.' + device + ', .customize-control .smvmt-responsive-btns > li.' + device).addClass('active');
            });
        },
        ...
        initAstBgControl: function( control, control_atts, name ) {
            ...
            controlContainer.on( 'click', '.background-image-upload-remove-button', function( e ) {
                ...
                controlContainer.find( '.more-settings' ).find('.message').html( smvmtCustomizerControlBackground.moreSettings );
...
( function( $ ) {
	...
	wp.customize.controlConstructor['smvmt-slider'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			// Handle the reset button.
			jQuery( '.smvmt-slider-reset' ).click( function() {
				var wrapper 		= jQuery( this ).closest( '.wrapper' ),
					input_range   	= wrapper.find( 'input[type=range]' ),
					input_number 	= wrapper.find( '.SMVMT_range_value .value' ),
					default_value	= input_range.data( 'reset_value' );
...
( function( $ ) {
	...
	wp.customize.controlConstructor['smvmt-sortable'] = wp.customize.Control.extend({
...
( function( $ ) {
	...
	AstTypography = {
		...
		_initFonts: function()
		{
			$( '.customize-control-smvmt-font-family select' ).each( function(e) {

				if( 'undefined' != typeof smvmt.customizer ) {
					var fonts = smvmt.customizer.settings.google_fonts;
					var optionName = $(this).data('name');
			...
			$( '.customize-control-smvmt-font-family select' ).each( AstTypography._initFont );
			// Added select2 for all font family & font variant.
			$('.customize-control-smvmt-font-family select, .customize-control-smvmt-font-variant select').selectWoo();

			$('.customize-control-smvmt-font-variant select').on('select2:unselecting', function (e) {
		...
		_setFontVarianttOptions: function( init )
		{
				...
				variantSavedField   = api.control( variants ).container.find( '.smvmt-font-variant-hidden-value' ),
				weightValue        = '',
				weightOptions       = '',
				...
				if ( 'Google' == selectedOptionGroup ) {
					variantSelect.parent().removeClass('smvmt-hide');
				} else{
					variantSelect.parent().addClass('smvmt-hide');
				}
