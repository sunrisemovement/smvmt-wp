 ...
 * @since 1.4.3
 */
if ( ! class_exists( 'SMVMT_Customizer_Control_Base' ) ) {

	/**
	 * Customizer Sanitizes Initial setup
	 */
	class SMVMT_Customizer_Control_Base {
		...
		public function enqueue_scripts() {
			...
			$file_rtl    = ( is_rtl() ) ? '-rtl' : '';
			$css_uri     = SMVMT_THEME_URI . 'inc/customizer/custom-controls/assets/css/' . $dir_name . '/';
			$js_uri      = SMVMT_THEME_URI . 'inc/customizer/custom-controls/assets/js/' . $dir_name . '/';

			wp_enqueue_style( 'custom-control-style' . $file_rtl, $css_uri . 'custom-controls' . $file_prefix . $file_rtl . '.css', null, SMVMT_THEME_VERSION );
			wp_enqueue_script( 'custom-control-script', $js_uri . 'custom-controls' . $file_prefix . '.js', array( 'jquery', 'customize-base', 'smvmt-color-alpha', 'jquery-ui-tabs', 'jquery-ui-sortable' ), SMVMT_THEME_VERSION, true );
		}
 ...
 */
new SMVMT_Customizer_Control_Base();
