 ...
 */
if ( ! class_exists( 'SMVMT_BB_Ultimate_Addon' ) ) :
	...
	class SMVMT_BB_Ultimate_Addon {
		...
		public function theme_color() {
			return smvmt_get_option( 'theme-color' );
		}
		...
		public function text_color() {
			return smvmt_get_option( 'text-color' );
		}
		...
		public function link_color() {
			return smvmt_get_option( 'link-color' );
		}
		...
		public function link_hover_color() {
			return smvmt_get_option( 'link-h-color' );
		}
		...
		public function button_font_family() {
			$font_family = str_replace( "'", '', smvmt_get_option( 'font-family-button' ) );
			$font_family = explode( ',', $font_family );
			return array(
				'family' => $font_family[0],
				'weight' => smvmt_get_option( 'font-weight-button' ),
			);
		}
		...
		public function default_type_button_font_size() {
			...
			$font_size_arr['desktop'] = smvmt_responsive_font( $theme_btn_font_size, 'desktop' );
			$font_size_arr['tablet']  = smvmt_responsive_font( $theme_btn_font_size, 'tablet' );
			$font_size_arr['mobile']  = smvmt_responsive_font( $theme_btn_font_size, 'mobile' );

			if ( empty( $font_size_arr['desktop'] ) ) {
				$font_size_arr['desktop'] = smvmt_responsive_font( $body_font_size, 'desktop' );
			}
			if ( empty( $font_size_arr['tablet'] ) ) {
				$font_size_arr['tablet'] = smvmt_responsive_font( $body_font_size, 'tablet' );
			}
			if ( empty( $font_size_arr['mobile'] ) ) {
				$font_size_arr['mobile'] = smvmt_responsive_font( $body_font_size, 'mobile' );
			}
		...
		public function default_type_button_line_height() {
			$theme_btn_body_line_height = smvmt_get_option( 'body-line-height', 1.85714285714286 );
			$theme_btn_line_height      = smvmt_get_option( 'theme-btn-line-height', $theme_btn_body_line_height );
		...
		public function button_text_color() {
			...
			$color       = smvmt_get_option( 'button-color' );
			if ( empty( $color ) ) {
		...
		public function button_bg_color() {
			return smvmt_get_option( 'button-bg-color' );
		}
		...
		public function button_bg_hover_color() {
			return smvmt_get_option( 'button-bg-h-color' );
		}
		...
		public function button_border_radius() {
			return smvmt_get_option( 'button-radius' );
		}
		...
		public function button_padding() {
			$padding   = '';
			$v_padding = smvmt_get_option( 'button-v-padding' );
			$h_padding = smvmt_get_option( 'button-h-padding' );
		...
		public function default_type_button_padding() {
			...
			$padding = smvmt_get_option( 'theme-button-padding' );
		...
		public function button_border_width() {
			...
			$btn_width     = array();
			$get_btn_width = smvmt_get_option( 'theme-button-border-group-border-size' );
		...
		public function button_border_color() {
			...
			$theme_color          = smvmt_get_option( 'theme-color' );
			$btn_bg_color         = smvmt_get_option( 'button-bg-color', $theme_color );
			$get_btn_border_color = smvmt_get_option( 'theme-button-border-group-border-color', $btn_bg_color );
		...
		public function button_border_hover_color() {
			...
			$link_hover_color       = smvmt_get_option( 'link-h-color' );
			$btn_bg_hover_color     = smvmt_get_option( 'button-bg-h-color', $link_hover_color );
			$get_btn_border_h_color = smvmt_get_option( 'theme-button-border-group-border-h-color', $btn_bg_hover_color );
 ...
 */
SMVMT_BB_Ultimate_Addon::get_instance();
