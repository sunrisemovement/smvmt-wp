 ...
 */
if ( ! class_exists( 'SMVMT_Elementor' ) ) :
	...
	class SMVMT_Elementor {
		...
		public function elementor_default_setting() {
			...
			if ( false == smvmt_enable_page_builder_compatibility() || 'post' == get_post_type() ) {
				return;
			}
			...
			if ( isset( $post ) && empty( $page_builder_flag ) && ( is_admin() || is_singular() ) ) {
				...
				if ( empty( $post->post_content ) && $this->is_elementor_activated( $id ) ) {
					...
					add_filter( 'smvmt_the_post_title_enabled', '__return_false' );
					add_filter( 'SMVMT_featured_image_enabled', '__return_false' );
				}
			}
		}
		...
		public function elementor_overlay_zindex() {
			...
			?>
			<style type="text/css" id="smvmt-elementor-overlay-css">
