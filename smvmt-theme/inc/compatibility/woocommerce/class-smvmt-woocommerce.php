...
/**
 * smvmt WooCommerce Compatibility
 */
if ( ! class_exists( 'SMVMT_Woocommerce' ) ) :
	...
	class SMVMT_Woocommerce {
		...
		public function __construct() {
			...
			require_once SMVMT_THEME_DIR . 'inc/compatibility/woocommerce/woocommerce-common-functions.php';
		 ...
		 * WooCommerce adds it's own product schema hence schema data from smvmt should be disabled here.
		 *
		 * @since 1.8.0
		...
		public function cart_page_upselles() {
			...
			$upselles_enabled = smvmt_get_option( 'enable-cart-upsells' );
			if ( ! $upselles_enabled ) {
		...
		public function woo_filter_style( $styles ) {
			...
			$css_uri = SMVMT_THEME_URI . 'assets/css/' . $dir_name . '/compatibility/woocommerce/';
			$key     = 'smvmt-woocommerce';
		...
		public function subcategory_count_markup( $content, $category ) {
			...
			$content = sprintf( // WPCS: XSS OK.
				...
				_nx( '<mark class="count">%1$s Product</mark>', '<mark class="count">%1$s Products</mark>', $category->count, 'product categories', 'smvmt' ),
		...
		public function product_flip_image() {
			...
			$hover_style = smvmt_get_option( 'shop-hover-style' );
			...
			if ( 'swap' === $hover_style ) {
				...
				if ( $attachment_ids ) {
					...
					echo apply_filters( 'SMVMT_woocommerce_product_flip_image', wp_get_attachment_image( reset( $attachment_ids ), $image_size, false, array( 'class' => 'show-on-hover' ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
			}
		}
		...
		public function shop_columns( $col ) {
			...
			$col = smvmt_get_option( 'shop-grids' );
			return $col['desktop'];
		}
		 ...
		 * @return boolean
		 */
		public function SMVMT_woo_is_subcategory( $category_id = null ) {
		...
		public function shop_no_of_products() {
			...
			if ( is_product_taxonomy() && 'subcategories' === $taxonomy_page_display ) {
				if ( $this->SMVMT_woo_is_subcategory() ) {
					$products = smvmt_get_option( 'shop-no-of-products' );
					return $products;
				}
		...
		public function single_product_class( $classes ) {
			...
			if ( is_product() && 0 == get_post_meta( get_the_ID(), '_wc_review_count', true ) ) {
				$classes[] = 'smvmt-woo-product-no-review';
			}
			...
			if ( is_shop() || is_product_taxonomy() ) {
				...
				if ( '' !== $hover_style ) {
					$classes[] = 'smvmt-woo-hover-' . $hover_style;
				}
			}
		...
		public function store_widgets_init() {
			register_sidebar(
				apply_filters(
					'SMVMT_woocommerce_shop_sidebar_init',
					array(
						'name'          => esc_html__( 'WooCommerce Sidebar', 'smvmt' ),
						'id'            => 'smvmt-woo-shop-sidebar',
			...
			register_sidebar(
				apply_filters(
					'SMVMT_woocommerce_single_sidebar_init',
					array(
						'name'          => esc_html__( 'Product Sidebar', 'smvmt' ),
						'id'            => 'smvmt-woo-single-sidebar',
						'description'   => __( 'This sidebar will be used on Single Product page.', 'smvmt' ),
						'before_widget' => '<div id="%1$s" class="widget %2$s">',
						'after_widget'  => '</div>',
						'before_title'  => '<h2 class="widget-title">',
						'after_title'   => '</h2>',
		...
		public function replace_store_sidebar( $sidebar ) {
			...
			if ( is_shop() || is_product_taxonomy() || is_checkout() || is_cart() || is_account_page() ) {
				$sidebar = 'smvmt-woo-shop-sidebar';
			} elseif ( is_product() ) {
				$sidebar = 'smvmt-woo-single-sidebar';
			}
		...
		public function store_sidebar_layout( $sidebar_layout ) {
			...
			if ( is_shop() || is_product_taxonomy() || is_checkout() || is_cart() || is_account_page() ) {
				...
				$woo_sidebar = smvmt_get_option( 'woocommerce-sidebar-layout' );
		...
		public function store_content_layout( $layout ) {
			...
			if ( is_woocommerce() || is_checkout() || is_cart() || is_account_page() ) {
				...
				$woo_layout = smvmt_get_option( 'woocommerce-content-layout' );
			...
			return apply_filters( 'smvmt_get_store_content_layout', $layout );
		}
		...
		public function shop_meta_option() {
			...
			if ( is_shop() ) {
				...
				$main_header_display = get_post_meta( $shop_page_id, 'smvmt-main-header-display', true );
		...
		public function shop_customization() {
			...
			if ( ! apply_filters( 'SMVMT_woo_shop_product_structure_override', false ) ) {

				add_action( 'woocommerce_before_shop_loop_item', 'SMVMT_woo_shop_thumbnail_wrap_start', 6 );
				/**
				 * Add sale flash before shop loop.
				 */
				add_action( 'woocommerce_before_shop_loop_item', 'woocommerce_show_product_loop_sale_flash', 9 );

				add_action( 'woocommerce_after_shop_loop_item', 'SMVMT_woo_shop_thumbnail_wrap_end', 8 );
				/**
				 * Add Out of Stock to the Shop page
				 */
				add_action( 'woocommerce_shop_loop_item_title', 'SMVMT_woo_shop_out_of_stock', 8 );
				 ...
				 */
				add_action( 'woocommerce_after_shop_loop_item', 'SMVMT_woo_woocommerce_shop_product_content' );
			}
		}
		...
		public function single_product_customization() {
			...
			if ( ! smvmt_get_option( 'single-product-breadcrumb-disable' ) ) {
		...
		public function before_main_content_start() {
			...
			<div id="primary" class="content-area primary">
				...
				<main id="main" class="site-main">
					<div class="smvmt-woocommerce-container">
			<?php
		}
		...
		public function before_main_content_end() {
			?>
					</div> <!-- .smvmt-woocommerce-container -->
				</main> <!-- #main -->
		...
		public function add_styles() {
			...
			$css_output = array(
				...
				'.woocommerce ul.products li.product .price, .woocommerce div.product p.price, .woocommerce div.product span.price, .widget_layered_nav_filters ul li.chosen a, .woocommerce-page ul.products li.product .smvmt-woo-product-category, .wc-layered-nav-rating a' => array(
					'color' => $text_color,
				),
				 ...
				 * Cart in menu
				 */
				'.smvmt-site-header-cart a'                 => array(
					'color' => esc_attr( $text_color ),
				),

				'.smvmt-site-header-cart a:focus, .smvmt-site-header-cart a:hover, .smvmt-site-header-cart .current-menu-item a' => array(
					'color' => esc_attr( $link_color ),
				),

				'.smvmt-cart-menu-wrap .count, .smvmt-cart-menu-wrap .count:after' => array(
					'border-color' => esc_attr( $link_color ),
					'color'        => esc_attr( $link_color ),
				),

				'.smvmt-cart-menu-wrap:hover .count'        => array(
					'color'            => esc_attr( $cart_h_color ),
					'background-color' => esc_attr( $link_color ),
				),

				'.smvmt-site-header-cart .widget_shopping_cart .total .woocommerce-Price-amount' => array(
					'color' => esc_attr( $link_color ),
				),

				'.woocommerce a.remove:hover, .smvmt-woocommerce-cart-menu .main-header-menu .woocommerce-custom-menu-item li:hover > a.remove:hover' => array(
					'color'            => esc_attr( $link_color ),
					'border-color'     => esc_attr( $link_color ),
				 ...
				 */
				'.smvmt-site-header-cart .widget_shopping_cart .buttons .button.checkout, .woocommerce .widget_shopping_cart .woocommerce-mini-cart__buttons .checkout.wc-forward' => array(
					'color'            => $btn_h_color,
					'border-color'     => $btn_bg_h_color,
					'background-color' => $btn_bg_h_color,
				),
				'.site-header .smvmt-site-header-cart-data .button.wc-forward, .site-header .smvmt-site-header-cart-data .button.wc-forward:hover' => array(
					'color' => $btn_color,
				),
				'.below-header-user-select .smvmt-site-header-cart .widget, .smvmt-above-header-section .smvmt-site-header-cart .widget a, .below-header-user-select .smvmt-site-header-cart .widget_shopping_cart a' => array(
					'color' => $text_color,
				),
				'.below-header-user-select .smvmt-site-header-cart .widget_shopping_cart a:hover, .smvmt-above-header-section .smvmt-site-header-cart .widget_shopping_cart a:hover, .below-header-user-select .smvmt-site-header-cart .widget_shopping_cart a.remove:hover, .smvmt-above-header-section .smvmt-site-header-cart .widget_shopping_cart a.remove:hover' => array(
					'color' => esc_attr( $link_color ),
				),
			);
				...
				if ( 'custom' === $woo_shop_archive_width ) :
					...
					$site_width  = array(
						'.smvmt-woo-shop-archive .site-content > .smvmt-container' => array(
							'max-width' => smvmt_get_css_value( $woo_shop_archive_max_width, 'px' ),
						),
					);
			...
			wp_add_inline_style( 'woocommerce-general', apply_filters( 'smvmt_theme_woocommerce_dynamic_css', $css_output ) );

			/**
			 * YITH WooCommerce Wishlist Style
			 */
			$yith_wcwl_main_style = array(
				'.yes-js.js_active .smvmt-plain-container.smvmt-single-post #primary' => array(
					'margin' => esc_attr( '4em 0' ),
				),
				'.js_active .smvmt-plain-container.smvmt-single-post .entry-header' => array(
			...
			/* Parse CSS from array() */
			$yith_wcwl_main_style = smvmt_parse_css( $yith_wcwl_main_style );
			...
			/* Parse CSS from array()*/
			$yith_wcwl_main_style .= smvmt_parse_css( $yith_wcwl_main_style_small, '', '768' );
		...
		public function customize_register( $wp_customize ) {
			 ...
			 */
			require SMVMT_THEME_DIR . 'inc/compatibility/woocommerce/customizer/class-smvmt-customizer-register-woo-section.php';

			/**
			 * Sections
			 */
			require SMVMT_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/class-smvmt-woo-shop-container-configs.php';
			require SMVMT_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/class-smvmt-woo-shop-sidebar-configs.php';
			require SMVMT_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/layout/class-smvmt-woo-shop-layout-configs.php';
			require SMVMT_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/layout/class-smvmt-woo-shop-single-layout-configs.php';
			require SMVMT_THEME_DIR . 'inc/compatibility/woocommerce/customizer/sections/layout/class-smvmt-woo-shop-cart-layout-configs.php';
		...
		public function woo_mini_cart_markup() {
			...
			$cart_menu_classes = apply_filters( 'SMVMT_cart_in_menu_class', array( 'smvmt-menu-cart-with-border' ) );

			ob_start();
			?>
			<div id="smvmt-site-header-cart" class="smvmt-site-header-cart <?php echo esc_attr( implode( ' ', $cart_menu_classes ) ); ?>">
				<div class="smvmt-site-header-cart-li <?php echo esc_attr( $class ); ?>">
					<?php $this->smvmt_get_cart_link(); ?>
				</div>
				<div class="smvmt-site-header-cart-data">
		 ...
		 * @since  1.0.0
		 */
		public function smvmt_get_cart_link() {

			$view_shopping_cart = apply_filters( 'SMVMT_woo_view_shopping_cart_title', __( 'View your shopping cart', 'smvmt' ) );
			?>
			<a class="cart-container" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php echo esc_attr( $view_shopping_cart ); ?>">

						<?php
						do_action( 'SMVMT_woo_header_cart_icons_before' );

						if ( apply_filters( 'SMVMT_woo_default_header_cart_icon', true ) ) {
							?>
							<div class="smvmt-cart-menu-wrap">
								<span class="count">
									<?php
									if ( apply_filters( 'SMVMT_woo_header_cart_total', true ) && null != WC()->cart ) {
						...
						do_action( 'SMVMT_woo_header_cart_icons_after' );
...
if ( apply_filters( 'smvmt_enable_woocommerce_integration', true ) ) {
	SMVMT_Woocommerce::get_instance();
}
