...
/**
 * Class SMVMT_Filesystem.
 */
class SMVMT_Filesystem {

	/**
	 * Store instance of SMVMT_Filesystem
	 *
	 * @since 2.1.0
	 * @var SMVMT_Filesystem
	 */
	protected static $instance = null;

	/**
	 * Get instance of SMVMT_Filesystem
	 *
	 * @since 2.1.0
	 * @return SMVMT_Filesystem
	 */
	...
	public function maybe_create_uploads_dir( $dir ) {
		...
		if ( ! file_exists( $dir ) ) {
			...
			$status = SMVMT_filesystem()->get_filesystem()->mkdir( $dir );

			// IF a directory cannot be created, return with false status.
			if ( false === $status ) {
				SMVMT_filesystem()->update_filesystem_access_status( $status );
				return false;
			}

			// Add an index file for security.
			SMVMT_filesystem()->get_filesystem()->put_contents( $dir . 'index.php', '' );
		}
	...
	public function update_filesystem_access_status( $status ) {
		smvmt_update_option( 'file-write-access', $status );
	}
	...
	public function can_access_filesystem() {
		return (bool) smvmt_get_option( 'file-write-access', true );
	}
	...
	public function reset_filesystem_access_status() {
		smvmt_delete_option( 'file-write-access' );
	}
	...
	public function get_uploads_dir( $assets_dir ) {
		...
		return apply_filters( 'smvmt_get_assets_uploads_dir', $dir_info );
	}
	...
	public function delete( $file, $recursive = false, $type = false ) {
		SMVMT_filesystem()->get_filesystem()->delete( $file, $recursive, $type );
	}
	...
	public function put_contents( $file_path, $style_data ) {
		return SMVMT_filesystem()->get_filesystem()->put_contents( $file_path, $style_data );
	}
	...
	public function get_contents( $file_path ) {
		return SMVMT_filesystem()->get_filesystem()->get_contents( $file_path );
	}
}
