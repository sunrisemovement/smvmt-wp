...
if ( ! class_exists( 'SMVMT_PB_Compatibility' ) ) {

	/**
	 * SMVMT_PB_Compatibility initial setup
	 *
	 * @since 1.0.13
	 */
	class SMVMT_PB_Compatibility {
		...
		public function maybe_run_pb_compatibility() {
			...
			$is_compatibility_completed = smvmt_get_option( '_SMVMT_pb_compatibility_completed', false );
		...
		public function page_builder_compatibility() {
			...
			$offset_comp = get_option( '_SMVMT_pb_compatibility_offset', false );
			$comp_time   = get_option( '_SMVMT_pb_compatibility_time', false );

			if ( ! $offset_comp || ! $comp_time ) {
				smvmt_update_option( '_SMVMT_pb_compatibility_completed', true );
				return;
			}
			...
			if ( $continue ) {
				$offset_comp += 30;
				update_option( '_SMVMT_pb_compatibility_offset', $offset_comp );
			} else {
				delete_option( '_SMVMT_pb_compatibility_offset' );
				delete_option( '_SMVMT_pb_compatibility_time' );
				smvmt_update_option( '_SMVMT_pb_compatibility_completed', true );
			}
		}
		...
		public function update_meta_values( $id ) {
			...
			if ( empty( $layout_flag ) ) {
				...
				if ( 'default' == $site_content ) {
					...
					if ( 'page' == $post_type ) {
						$site_content = smvmt_get_option( 'single-page-content-layout', '' );
					} elseif ( 'post' == $post_type ) {
						$site_content = smvmt_get_option( 'single-post-content-layout', '' );
					}

					if ( 'default' == $site_content ) {
						$site_content = smvmt_get_option( 'site-content-layout', '' );
					}
				}
 ...
 */
SMVMT_PB_Compatibility::get_instance();
