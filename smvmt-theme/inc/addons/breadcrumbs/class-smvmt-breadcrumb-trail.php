 ...
 * @return string
 */
function smvmt_get_breadcrumb( $echo = true ) {

	if ( ! $echo ) {
		return '<div class="smvmt-breadcrumbs-wrapper">
			<div class="smvmt-breadcrumbs-inner">' .
				smvmt_get_selected_breadcrumb( $echo ) .
			'</div>
		</div>';
	}

	?>
	<div class="smvmt-breadcrumbs-wrapper">
		<div class="smvmt-breadcrumbs-inner">
			<?php smvmt_get_selected_breadcrumb( $echo ); ?>
		</div>
	</div>
	<?php
 ...
 */
function smvmt_get_selected_breadcrumb( $echo = true ) {
	...
	if ( function_exists( 'yoast_breadcrumb' ) && true === $wpseo_option['breadcrumbs-enable'] && $breadcrumb_source && 'yosmvmt-seo-breadcrumbs' == $breadcrumb_source ) {
		...
		// Load default smvmt breadcrumb if none selected.
		return smvmt_get_breadcrumb_trail( $echo );
	}
}


/**
 * Deprecating SMVMT_breadcrumb_trail function.
 *
 * @since 1.8.1
 * @deprecated 1.8.1 Use smvmt_get_breadcrumb()
 * @param array  $args List of args.
 * @see SMVMT_breadcrumb_trail()
 *
 * @return string new breadcrumb function.
 */
function SMVMT_breadcrumb_trail( $args = array() ) {
	_deprecated_function( __FUNCTION__, '1.8.1', 'smvmt_get_breadcrumb()' );
 ...
 * @return string Selected Breadcrumb.
 */
function smvmt_get_breadcrumb_trail( $echo = true ) {

	$defaults = array(
		'before'      => '<div class="smvmt-breadcrumbs">',
		'after'       => '</div>',
		'show_browse' => false,
		'echo'        => $echo,
	);

	$args = apply_filters( 'SMVMT_breadcrumb_trail_args', $defaults );

	$breadcrumb = apply_filters( 'SMVMT_breadcrumb_trail_object', null, $args );

	if ( ! is_object( $breadcrumb ) )
		$breadcrumb = new SMVMT_Breadcrumb_Trail( $args );
...
class SMVMT_Breadcrumb_Trail {
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'SMVMT_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );
	...
	public function trail() {
		...
		$breadcrumb = apply_filters( 'SMVMT_breadcrumb_trail', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'smvmt' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'smvmt' ),
			'home'                => esc_html__( 'Home',                                  'smvmt' ),
			'error_404'           => esc_html__( '404 Not Found',                         'smvmt' ),
			'archives'            => esc_html__( 'Archives',                              'smvmt' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s',                'smvmt' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'smvmt' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s',                       'smvmt' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'smvmt' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'smvmt' ),
		...
		$this->labels = apply_filters( 'SMVMT_breadcrumb_trail_labels', wp_parse_args( $this->args['labels'], $defaults ) );
	}
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'SMVMT_breadcrumb_trail_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	}
	...
	protected function add_items() {
		...
		$this->items = array_unique( apply_filters( 'SMVMT_breadcrumb_trail_items', $this->items, $this->args ) );
	}
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'smvmt' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'smvmt' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'smvmt' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'smvmt' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'smvmt' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'smvmt' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'smvmt' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'smvmt' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'smvmt' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'smvmt' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'smvmt' ) ) ) );
