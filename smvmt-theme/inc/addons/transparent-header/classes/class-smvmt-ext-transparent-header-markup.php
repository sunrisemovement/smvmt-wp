...
if ( ! class_exists( 'SMVMT_ExtTransparenty_Header_Markup' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class SMVMT_Ext_Transparent_Header_Markup {
		...
		public function add_body_class( $classes ) {
			...
			$inherit_desk_logo              = smvmt_get_option( 'different-transparent-logo', false );
			$transparent_header_logo        = smvmt_get_option( 'transparent-header-logo', true );
			$transparent_header_logo_retina = smvmt_get_option( 'transparent-header-retina-logo', true );

			if ( '1' == $inherit_desk_logo && ( '' !== $transparent_header_logo || '' !== $transparent_header_logo_retina ) ) {
				$classes[] = 'smvmt-replace-site-logo-transparent';
			}

			if ( '1' !== $inherit_desk_logo ) {
				$classes[] = 'smvmt-inherit-site-logo-transparent';
			}

			/**
			 * Add class 'smvmt-theme-transparent-header'
			 */

			if ( self::is_transparent_header() ) {
				$classes[] = 'smvmt-theme-transparent-header';
			}
		...
		/**
		 * smvmt check if transparent header is enabled.
		 *
		...
		public static function is_transparent_header() {
			...
			// Transparent Header.
			$enable_trans_header = smvmt_get_option( 'transparent-header-enable' );
			$trans_meta_option   = smvmt_get_option_meta( 'theme-transparent-header-meta', 'default' );

			if ( $enable_trans_header ) {

				if ( ( is_archive() || is_search() || is_404() ) && '1' == smvmt_get_option( 'transparent-header-disable-archive' ) ) {
					$enable_trans_header = false;
				}

				if ( is_home() && '1' == smvmt_get_option( 'transparent-header-disable-index' ) && ( 'posts' !== get_option( 'show_on_front' ) ) ) {
					$enable_trans_header = false;
				}

				if ( is_front_page() && 'posts' == get_option( 'show_on_front' ) && '1' == smvmt_get_option( 'transparent-header-disable-latest-posts-index' ) ) {
					$enable_trans_header = false;
				}

				if ( is_page() && '1' == smvmt_get_option( 'transparent-header-disable-page' ) ) {
					$enable_trans_header = false;
				}

				if ( is_single() && '1' == smvmt_get_option( 'transparent-header-disable-posts' ) ) {
					$enable_trans_header = false;
				}
			}

			if ( class_exists( 'SMVMT_Woocommerce' ) ) {
				if ( is_product() && '1' == smvmt_get_option( 'transparent-header-disable-woo-products' ) ) {
					$enable_trans_header = false;
				}
			}
			...
			return apply_filters( 'smvmt_is_transparent_header', $enable_trans_header );
		}
		...
		public function transparent_custom_logo( $html, $blog_id ) {
			...
			$trans_logo                 = smvmt_get_option( 'transparent-header-logo' );
			...
			if ( '' !== $trans_logo ) {
				...
				$size = 'smvmt-transparent-logo-size';
				...
				if ( 'mobile' === $transparent_header_devices ) {
					...
					$html .= sprintf(
						'<a href="%1$s" class="custom-logo-link smvmt-transparent-desktop-logo" rel="home" itemprop="url">%2$s</a>',
						esc_url( home_url( '/' ) ),
						wp_get_attachment_image(
				...
				if ( 'desktop' === $transparent_header_devices ) {
					...
					$html .= sprintf(
						'<a href="%1$s" class="custom-logo-link smvmt-transparent-mobile-logo" rel="home" itemprop="url">%2$s</a>',
						esc_url( home_url( '/' ) ),
						wp_get_attachment_image(
							get_theme_mod( 'custom_logo' ),
							$size,
		...
		public function replace_trans_header_attr( $attr, $attachment, $size ) {
			...
			if ( $custom_logo_id == $attachment->ID ) {
				...
				$attach_data = array();
				if ( ! is_customize_preview() ) {
					$attach_data = wp_get_attachment_image_src( $attachment->ID, 'smvmt-transparent-logo-size' );
				...
				$diff_retina_logo = smvmt_get_option( 'different-transparent-retina-logo' );

				if ( '1' == $diff_retina_logo ) {

					$retina_logo = smvmt_get_option( 'transparent-header-retina-logo' );

					$attr['srcset'] = '';

					if ( apply_filters( 'SMVMT_transparent_header_retina', true ) && '' !== $retina_logo ) {
			...
			<?php if ( $show_meta_field ) { ?>
				<div class="transparent-header-wrapper">
					<p class="post-attributes-label-wrapper">
						<strong> <?php esc_html_e( 'Transparent Header', 'smvmt' ); ?> </strong><br/>
					</p>
					<select name="theme-transparent-header-meta" id="theme-transparent-header-meta">
						<option value="default" <?php selected( $trans_header_meta, 'default' ); ?>> <?php esc_html_e( 'Customizer Setting', 'smvmt' ); ?> </option>
						<option value="enabled" <?php selected( $trans_header_meta, 'enabled' ); ?>> <?php esc_html_e( 'Enabled', 'smvmt' ); ?> </option>
						<option value="disabled" <?php selected( $trans_header_meta, 'disabled' ); ?>> <?php esc_html_e( 'Disabled', 'smvmt' ); ?> </option>
					</select>
				</div>
			<?php } ?>
		...
		public function customizer_save() {
			...
			if ( '' !== $trans_logo ) {
				...
				SMVMT_Customizer::generate_logo_by_width( $trans_logo_id );
		...
		public function transparent_logo_image_sizes( $sizes, $metadata ) {
			...
			$logo_width = smvmt_get_option( 'transparent-header-logo-width' );

			if ( is_array( $sizes ) && '' != $logo_width['desktop'] ) {
				$max_value                          = max( $logo_width );
				$sizes['smvmt-transparent-logo-size'] = array(
					'width'  => (int) $max_value,
					'height' => 0,
					'crop'   => false,
				);
...
*/
SMVMT_Ext_Transparent_Header_Markup::get_instance();
