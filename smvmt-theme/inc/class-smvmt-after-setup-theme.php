 ...
 *
 * smvmt is a very powerful theme and virtually anything can be customized
 * via a child theme.
 *
 * @package     smvmt
 * @author      smvmt
...
/**
 * SMVMT_After_Setup_Theme initial setup
 *
 * @since 1.0.0
 */
if ( ! class_exists( 'SMVMT_After_Setup_Theme' ) ) {

	/**
	 * SMVMT_After_Setup_Theme initial setup
	 */
	class SMVMT_After_Setup_Theme {
		...
		public function setup_theme() {
			...
			do_action( 'SMVMT_class_loaded' );

			/**
			 * Make theme available for translation.
			 * Translations can be filed in the /languages/ directory.
			 * If you're building a theme based on Next, use a find and replace
			 * to change 'smvmt' to the name of your theme in all the template files.
			 */
			load_theme_textdomain( 'smvmt', SMVMT_THEME_DIR . '/languages' );
			...
			if ( apply_filters( 'smvmt_theme_editor_style', true ) ) {
				add_editor_style( 'assets/css/' . $dir_name . '/editor-style' . $file_prefix . '.css' );
			}

			if ( apply_filters( 'SMVMT_fullwidth_oembed', true ) ) {
			...
			// Native AMP Support.
			if ( true === apply_filters( 'SMVMT_amp_support', true ) ) {
				add_theme_support(
					'amp',
					apply_filters(
						'SMVMT_amp_theme_features',
						array(
							'paired' => true,
						)
					)
		...
		public function responsive_oembed_wrapper( $html, $url, $attr ) {
			...
			$add_SMVMT_oembed_wrapper = apply_filters( 'smvmt_responsive_oembed_wrapper_enable', true );

			$allowed_providers = apply_filters(
				'SMVMT_allowed_fullwidth_oembed_providers',
				array(
					'vimeo.com',
					'youtube.com',
					'youtu.be',
			...
			if ( SMVMT_strposa( $url, $allowed_providers ) ) {
				if ( $add_SMVMT_oembed_wrapper ) {
					$html = ( '' !== $html ) ? '<div class="smvmt-oembed-container">' . $html . '</div>' : '';
				}
			}
 ...
 */
SMVMT_After_Setup_Theme::get_instance();
