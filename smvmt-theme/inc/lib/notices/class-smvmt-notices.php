 ...
 * smvmt Sites Notices
 *
 * Closing notice on click on `smvmt-notice-close` class.
 *
 ...
 * `<a href="#" data-repeat-notice-after="<?php echo MONTH_IN_SECONDS; ?>" class="smvmt-notice-close">`
...
if ( ! class_exists( 'SMVMT_Notices' ) ) :
	...
	class SMVMT_Notices {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_smvmt-notice-dismiss', array( $this, 'dismiss_notice' ) );
		...
		public function dismiss_notice() {
			...
			if ( false === wp_verify_nonce( $nonce, 'smvmt-notices' ) ) {
				wp_send_json_error( esc_html_e( 'WordPress Nonce not validated.', 'smvmt' ) );
			}
		...
		public function enqueue_scripts() {
			wp_register_script( 'smvmt-notices', self::_get_uri() . 'notices.js', array( 'jquery' ), self::$version, true );
			wp_localize_script(
				'smvmt-notices',
				'smvmtNotices',
				array(
					'_notice_nonce' => wp_create_nonce( 'smvmt-notices' ),
				)
			);
		}
		...
		public function show_notices() {
			...
			$defaults = array(
				'id'                         => '',      // Optional, Notice ID. If empty it set `smvmt-notices-id-<$array-index>`.
				...
				'display-with-other-notices' => true,    // Should the notice be displayed if other notices  are being displayed from SMVMT_Notices.
				'is_dismissible'             => true,
			);
		...
		public static function markup( $notice = array() ) {
			...
			wp_enqueue_script( 'smvmt-notices' );

			do_action( 'SMVMT_notice_before_markup' );

			do_action( "SMVMT_notice_before_markup_{$notice['id']}" );
			...
			<div id="<?php echo esc_attr( $notice['id'] ); ?>" class="<?php echo esc_attr( $notice['classes'] ); ?>" data-repeat-notice-after="<?php echo esc_attr( $notice['repeat-notice-after'] ); ?>">
				<div class="notice-container">
					<?php do_action( "SMVMT_notice_inside_markup_{$notice['id']}" ); ?>
					<?php echo wp_kses_post( $notice['message'] ); ?>
				</div>
			</div>
			<?php

			do_action( "SMVMT_notice_after_markup_{$notice['id']}" );

			do_action( 'SMVMT_notice_after_markup' );
		...
		private static function get_wrap_classes( $notice ) {
			$classes = array( 'smvmt-notice', 'notice' );
