...
(function ($) {
    ...
    var SMVMT_Customizer = {
        ...
        hasDependentControls: function( control_id ) {
            ...
            $.each(smvmt.config, function (index, val) {
        ...
        checkControlVisibility: function (control, id) {
            ...
            if ( !_.isUndefined( control ) ) {
                ...
                if ( 'undefined' != typeof smvmt.config[id] ) {
                    var check = false;
                    var required_param = smvmt.config[id];
                    ...
                    if ( 'undefined' !== typeof conditions ) {
                        ...
                        if (!check) {
                            control.container.addClass('smvmt-hide');
                        } else {
                            control.container.removeClass('smvmt-hide');
                        }
                    }
                }
            }
        ...
        checkDependency: function (conditions, values, compare_operator) {
            ...
            if ( _.isString( test ) ) {
                ...
                if ( !_.isUndefined( smvmt.config[test] ) ) {

                    var conditions = !_.isUndefined(smvmt.config[test]['conditions']) ? smvmt.config[test]['conditions'] : smvmt.config[test];
                    var operator = !_.isUndefined(smvmt.config[test]['operator']) ? smvmt.config[test]['operator'] : 'AND';
                    ...
                    if ( !_.isUndefined( conditions ) ) {
                        ...
                        if ( ! control.checkDependency( conditions, values, operator ) ) {
                            ...
                            control_obj.container.removeClass('smvmt-hide');
                        }
                    }
                }
                ...
                $.each( conditions, function ( index, val ) {
                    ...
                    if ( 'undefined' !== typeof smvmt.config[cond_key] ) {

                        var conditions = !_.isUndefined(smvmt.config[cond_key]['conditions']) ? smvmt.config[cond_key]['conditions'] : smvmt.config[cond_key];
                        var operator = !_.isUndefined(smvmt.config[cond_key]['operator']) ? smvmt.config[cond_key]['operator'] : 'AND';
            ...
            $('ul.accordion-section.control-section-smvmt_section').each(function () {
                ...
                if ( controls.length > 0 ) {
                    ...
                    controls.each(function () {
                        ...
                        if ( ! $(this).hasClass('smvmt-hide') && $(this).css('display') != 'none' ) {
                            visibleIt = true;
                        }
                    });

                    if (!visibleIt) {
                        $('.control-section[aria-owns="' + parentId + '"]').addClass('smvmt-hide');
                    } else {
                        $('.control-section[aria-owns="' + parentId + '"]').removeClass('smvmt-hide');
                    }
                }
            });
    ...
    $(function () { SMVMT_Customizer.init(); });
